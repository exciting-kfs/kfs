...
impl<T: ListNode<T> + Sync + Eq> List<T> {
	...
	pub fn push_back(&mut self, node: Arc<T>) {
		if let Some(next) = self.head.as_mut().and_then(|h| h.upgrade()) {
			...
			self.head = Some(Arc::downgrade(&node));
		}
	}

	pub fn pop_front(&mut self) -> Option<Arc<T>> {
		let n = self.head.as_mut().and_then(|n| n.upgrade());
	...
	pub fn first(&self) -> Option<Arc<T>> {
		self.head.as_ref().and_then(|h| h.upgrade())
	}

	pub fn move_to_back(&mut self, node: Arc<T>) {
		if let Some(head) = self.head.as_ref().and_then(|h| h.upgrade()) {
	...
	pub fn remove(&mut self, node: Arc<T>) {
		if let Some(head) = self.head.as_mut().and_then(|h| h.upgrade()) {
			...
			if node == head {
				if node == next {
					...
					self.head = Some(next_weak.clone());
				}
			}
