...
impl SuperBlock {
	...
	fn parse_to_inode(
		...
		unsafe {
			...
			Arc::new(LockRW::new(Inode::from_info(
				inum,
				info.clone(),
				self.clone(),
			)))
	...
	fn sync_self(&self) -> Result<(), Errno> {
		const SYNC_INTERVAL: u32 = 30; // hmm..
...
impl vfs::SuperBlock for SuperBlock {
	fn sync(&self) {
		let _ = self.try_sync_icache();
		let _ = self.sync_self();
