...
use crate::fs::vfs::{lookup_entry, AccessFlag, CreationFlag, IOFlag, Permission, VfsEntry};
...
fn lookup_or_create(
	...
	let base_entry = lookup_entry(path, task)?;

	let entry = match file {
		Some(ref name) => base_entry
			.clone()
			.downcast_dir()
			.and_then(|dir| dir.lookup(name, task)),
		None => Ok(base_entry.clone()),
	};

	match entry {
		// directory / file already exists.
		Ok(e) => match creation_flags.contains(CreationFlag::O_EXCL) {
...
pub fn sys_open2(path: usize, flags: i32, perm: u32) -> Result<usize, Errno> {
	...
	let ent = match creation_flags.contains(CreationFlag::O_CREAT) {
		...
		false => lookup_entry(path, current),
	}?;
