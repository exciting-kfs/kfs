...
pub fn sys_bind(socket_fd: usize, addr: usize, addr_len: usize) -> Result<usize, Errno> {
	let current = unsafe { CURRENT.get_ref() };

	let mut addr = UnknownSocketAddress::new_read_only(addr, addr_len, current)
	...
	handle.bind(&mut addr, current).map(|_| 0)
}
