...
static unsigned long parse_rounds(const char *str, char **end) {
	unsigned long n = 0;
	for (; ft_isdigit(*str); ++str) {
		n = (n * 10) + (*str - '0');
		if (n >= ROUNDS_MAX)
			n = ROUNDS_MAX;
	}
...
static void write_ul(char *buf, unsigned long n) {
	char temp[16];
...
static char *sha512crypt(const char *key, const char *setting, char *output)
{
	...
	if (ft_strncmp(setting, "$6$", 3) != 0)
		return 0;
	salt = setting + 3;

	r = ROUNDS_DEFAULT;
	if (ft_strncmp(salt, "rounds=", sizeof "rounds=" - 1) == 0) {
		unsigned long u;
		char *end;
		...
		ft_strlcat(rounds, "rounds=", sizeof(rounds));
		write_ul(rounds + sizeof("rounds=") - 1, r);
		ft_strlcat(rounds, "$", sizeof(rounds));
...
char *__crypt_sha512(const char *key, const char *setting, char *output)
{
	...
	if (!p || q != testbuf || ft_memcmp(testbuf, testhash, sizeof testhash))
		return "*";
	return p;
}